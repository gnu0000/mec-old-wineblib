 ...
 * Craig Fitzgerald
 *
 * This file provides the EbOpen function which is a superset of FilOpen
...
PLDESC OpenLib (PSZ pszLib)
   ...
   if (!(fp = FilOpen (pszLib, "rb")))
      return SetLibErr (6);

   if (FilRead (szBuff, 1, HEADERSIZE, fp) != HEADERSIZE)
      {
      FilClose (fp);
      return SetLibErr (1);
      }
   ...
   pld->pszDesc = calloc (1, LIBDESCRSIZE);
   ...
   FilSeek (fp, pld->ulOffset, SEEK_SET);
...
PFDESC ReadFileInfo (PLDESC pld, BOOL bSkipData)
   ...
   *p = FilReadUShort (pld->fp);
   ...
   FilSeek (pld->fp, pfd->ulOffset + (bSkipData ? pfd->ulSize + EBLMARKSIZE : 0), SEEK_SET);
...
void SkipFileData (PFDESC pfd)
   {
   FilSeek (pfd->pld->fp, pfd->ulOffset + pfd->ulSize + EBLMARKSIZE, SEEK_SET);
   }
...
//   if (pfdPrev)
//      {
//      FilSeek (pld->fp, pfdPrev->ulOffset, SEEK_SET);
//      SkipFileData (pfdPrev);
//      if (bFreeOld)
//         FreePFD (pfdPrev);
//      }
//   else
//      FilSeek (pld->fp, pld->ulOffset, SEEK_SET);
//
...
//
//   if (!*szEBLLIB)
//      return FilOpen (pszFile, pszMode);
//
