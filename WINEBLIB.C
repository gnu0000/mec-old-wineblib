...
#include <windows.h>
#else
// define windows functions as macros that do nothing

#define IsBadWritePtr(p,l) (0)

#endif 


static char szTMPLIB [50];
...
#define TESTFILE(s) { OFSTRUCT of; HFILE hf; of.cBytes = sizeof of; \
hf = OpenFile ((s), &of, OF_EXIST);\
printf ("OpenFile: hf = %d (%x), of.fFixed = %d, uError = %d (%x), szPathName = %s",\
hf, hf, of.fFixedDisk, of.nErrCode, of.nErrCode, of.szPathName);}



CHAR szLIB [256];
CHAR szWORKBUFF [35500];
PSZ  pszWORKBUFF = szWORKBUFF;
...
{ /* 0 */ "No Error",
  ...
  /* 13*/ "File Exists",
  /* 14*/ "Can't make path",
  /* 15*/ "Destination variable too small!",
  /* 16*/ NULL};
...
#define LOGFILE "C:\\WINEBLIB.LOG"
static char szTemp [10240];

int __cdecl printf (PSZ pszFormat, ...)
   {
   HFILE hFile; 
   va_list va;
   int i;
   ...
   i = _vsnprintf (szTemp, sizeof szTemp, pszFormat, va);
   ...
   if (i == -1)
	...
	hFile = _lopen (LOGFILE, OF_WRITE);
	_llseek (hFile, 0L, 2);
	_lwrite (hFile, "WINEBLIB >>>> ", 12);
	_lwrite (hFile, szTemp, lstrlen (szTemp));
	_lwrite (hFile, "\r\n", 2);
	_lclose (hFile); 
...
// mdh: evil...
// evil FreeList macro, frees the hideous global fList.

#define FreeList() (fList=FreePFDESC(fList))
...
static UINT SetErr (UINT uErr, PSZ pszLocus)
   {
   printf ("SetErr: current err: %d. New err: %d. Locus:%s", GLOBALERROR, uErr, pszLocus);
   if (!uErr)
      GLOBALERROR = 0;
   else if (!GLOBALERROR)
      GLOBALERROR = uErr;
   ...
   while (ulSize)
      ...
      if (uCompression)
         ...
         if ((ulSegment = (ULONG)FilReadUShort (fpIn)) > ulSize)
            return SetErr (EBLERR_BADSIZE, "CopyFile1");
      ...
      while (ulSegment)
         ...
         uIOBytes = FilRead (pszWORKBUFF, 1, uPiece, fpIn);
         if (uPiece != uIOBytes)
            return SetErr (EBLERR_INPUTEOF, "CopyFile2");

         if (fpOut)
            {
            uIOBytes = FilWrite (pszWORKBUFF, 1, uPiece, fpOut);
            if (uPiece != uIOBytes)
               return SetErr (EBLERR_OUTPUTEOF, "CopyFile3");
            }
         ulSegment -= uPiece;
...
//   ulREADLEN = 0;
   ...
   if (ulTotalOut != ulOutSize)
      return SetErr (EBLERR_BADSIZE, "UncompressFile");
   return 0;
   }
...
static UINT TestFile (PFDESC pfd)
   ...
   if (Cmp2GetCRC (1) != pfd->ulCRC)
      return SetErr (EBLERR_CRCERROR, "TestFile");
   return 0;
   }
...
static UINT WriteToFile (PFDESC pfd, BOOL bSetFilePos)
   ...
   if (!access (pszName, 0) && (iOVERWRITE < 0))
      return SetErr (EBLERR_FILEEXISTS, "WriteToFile -- exists");
   ...
   if (*szPath && access (szPath, 0))
      if (!DirMakePath (szPath))
         return SetErr (EBLERR_CANNOTMAKEPATH, "WriteToFile -- access!");

   if (!(fpOut = FilOpen (pszName, (bAPPEND ? "ab" : "wb"))))  // mdh
      return SetErr (EBLERR_CANTOPEN, pszName);

   if (bSetFilePos)
      FilSeek (pfd->pld->fp, pfd->ulOffset, SEEK_SET);
   ...
   if (!uRet && (Cmp2GetCRC (1) != pfd->ulCRC))
      uRet = SetErr (EBLERR_CRCERROR, "WriteToFile");
...
static PFDESC FreePFDESC (PFDESC pfd) //mdh
   {
   PFDESC pfdTemp;
   ...
   for (; pfd; pfd = pfdTemp)
      {
      pfdTemp = pfd->Next;
      MemFreeData (pfd);   // don't free pfd->pld!
      }
...
static void UpdateFileHeader (FILE *fp, ULONG ulFilePos, ULONG ulOutSize, ULONG ulLen, ULONG ulWRITECRC)
   {
   FilPushPos (fp);
   FilSeek (fp, ulFilePos + SIZEOFFSET, SEEK_SET);
...
static UINT WriteTheDamnFile (PLDESC pldOut, PFDESC pfd, ULONG ulFilePos)
   ...
   if (!(fpIn = FilOpen (pfd->szName, "rb")))
      return SetErr (EBLERR_CANTOPEN, pfd->szName);
...
static UINT WriteLibFromList (PLDESC pldOut)
   ...
   for (i=0, pfd = fList; pfd; pfd = pfd->Next, i++)
      ...
      if (!uRet) // ok
         ...
         {
         FilSeek (pldOut->fp, ulFilePos, SEEK_SET); // rewind over file
         pfd->uMethod = STORE;
         if (!WriteTheDamnFile (pldOut, pfd, ulFilePos))
            iFiles++;
         else
            FilSeek (pldOut->fp, ulFilePos, SEEK_SET);
         }
      else // (uRet == 2)  error
         {
         FilSeek (pldOut->fp, ulFilePos, SEEK_SET); // rewind over error'd file
         }
      }
   WriteMark  (pldOut->fp);
   ulCurrPos = FilTell (pldOut->fp);
   FilSeek (pldOut->fp, FILELENOFFSET, SEEK_SET);
...
static PLDESC MakePLDESC (PSZ pszDesc) 
   ...
   if (!pld)
      {
      SetErr (EBLERR_NOMEM, "MakePLDESC!");
      return NULL;
      }
...
static PLDESC MakeLibFile (PSZ pszLib, PSZ pszDesc)
   ...
   if (!(pld = MakePLDESC (pszDesc)))
      {
      SetErr (EBLERR_NOMEM, "MakeLibFile!");
      return NULL;
      }

   if (!(pld->fp = FilOpen (pszLib, "wb")))
      {
      SetErr (EBLERR_CANTOPEN, pszLib);
      FreePLDESC (pld);
      return FreePLDESC (pld);
      }

   FilWrite (LIBHEADER, 1, HEADERSIZE, pld->fp);
...
static int ListLib (PSZ pszLib, PSZ pszFileSpec, PSZ pszOutFile)  // fix1
   {
   PLDESC pld;
   PFDESC pfd;
   UINT   i;
   ...
   if (!(pld = OpenLib (pszLib)))
      return SetErr (GetLibErr (), pszLib);

   if (!(fp = fopen (pszOutFile, "wt")))
      {
      FilClose (pld->fp);
      FreePLDESC (pld);
      return SetErr (EBLERR_CANTOPEN, pszLib);
      }
   ...
   for (i=0; i<pld->uCount; i++)
      ...
      if (!(pfd = ReadFileInfo (pld, TRUE)))
         ...
         FilClose (pld->fp);
         FreePLDESC (pld);
         return SetErr (EBLERR_CANTREAD, "ListLib");
         }
...
static int TestLib (PSZ pszLib)  // fix1
   {
   PLDESC pld;
   PFDESC pfd;
   UINT i;
   ...
   if (!pld->uCount)
      ...
      FilClose (pld->fp);
      FreePLDESC (pld);
      return SetErr(EBLERR_NOMATCH, "TestLib");
      }
   ...
   for (i=0; i<pld->uCount; i++)
      ...
      if (!(pfd = ReadFileInfo (pld, FALSE)))
         ...
         FilClose (pld->fp);
         FreePLDESC (pld);
         return SetErr (EBLERR_CANTREAD, "TestLib2");
         }
...
static int QuickTestLib (PSZ pszLib)  // fix1
   {
   PLDESC pld;
...
static UINT AddMatchingFiles (PSZ pszPath, PSZ pszMatch) // fix1
   {
   UINT uMatchCt, uAtts;
   PFINFO pfo = NULL;
   PFDESC pfd;
   ...
   for (uMatchCt=0; pfo = DirFindFile (szSearchSpec, uAtts, pfo); uMatchCt++)
      {
      if (!stricmp (pfo->szName, szLIB) || !stricmp (pfo->szName, szTMPLIB))
         continue;

      pfd = calloc (1, sizeof (FDESC));
      if (!pfd) // mdh
         {
         // oh crap. out of memory!
         FreeList ();
         return 0;
         }
...
static int Dogbert (PSZ pszMatchSpec) // fix1
   {
   UINT uFiles;
...
static int RecurseAddMatchingFiles (PSZ pszMatchSpec) // fix1
   {
...
static UINT AddLib (PSZ pszLib, PSZ pszFileSpec, BOOL bMove) // fix
   {
   PLDESC pldOut, pldIn;
   PFDESC pfd;
   PSZ    pszOutFile;
   ...
   if (!(pldIn = OpenLib (pszLib)))        // could not open lib or bad lib
      {
      if (GetLibErr () != EBLERR_CANTOPEN) // no lib file
         return SetErr (GetLibErr (), pszLib);
      pszOutFile = pszLib;
      }
   else  // opened lib ok
      ...
      for (i=0; i<pldIn->uCount; i++)
         ...
         if (!(pfd = ReadFileInfo (pldIn, TRUE)))
            ...
            FreePLDESC (pldIn);
            FreeList ();
            return SetErr (EBLERR_CANTREAD, "AddLib2");
            }
         pfd->uMode = LIB;
         AddToFileList (pfd);
         }
   ...
   if (!RecurseAddMatchingFiles (pszFileSpec))
      ...
      FreePLDESC (pldOut);
      FreeList ();
		return SetErr (EBLERR_NOMATCH, "AddLib3");
      }
...
static UINT DelLib (PSZ pszLib, PSZ pszFileSpec)  // fix1
   {
   PFDESC fTmp, pfd;
   PLDESC pldOut, pldIn;
   UINT   i, uDeleteCount = 0;

   if (!pszFileSpec)  //mdh moved from after pldIn= line below
      return 0;
   ...
   for (i=0; i<pldIn->uCount; i++)
      ...
      if (!(pfd = ReadFileInfo (pldIn, TRUE)))
         ...
         FreePLDESC (pldIn);
         FreeList ();
         return SetErr (EBLERR_CANTREAD, "DelLib");
         }
   ...
   if (!uDeleteCount)
      ...
      FreePLDESC (pldIn);
      FreeList ();
      return SetErr (EBLERR_NOMATCH, "DelLib2");
      }

   if (!(pldOut = MakeLibFile (szTMPLIB, pldIn->pszDesc)))
      {
      FilClose (pldIn->fp);
      FreePLDESC (pldIn);
      FreeList ();
...
static UINT XLib (PSZ pszLib, PSZ pszFileSpec) // fix1
   {
   PFDESC pfd;
   PLDESC pldIn;
   ...
   for (j=0; j<pldIn->uCount; j++)
      ...
      if (!(pfd = ReadFileInfo (pldIn, FALSE)))
         ...
         FilClose (pldIn->fp);
         FreePLDESC (pldIn);
         return SetErr (EBLERR_CANTREAD, "XLib");
         }
...
static UINT DescLib (PSZ pszLib, PSZ pszDesc) // fix1
   {
   UINT   i;
   PLDESC pldOut, pldIn;
   PFDESC pfd;
   ...
   /*--- open output lib ---*/
   if (!(pldOut = MakeLibFile (szTMPLIB, pszDesc)))
      {
      FilClose (pldIn->fp);
      FreePLDESC (pldIn);
      return GetErr ();
   ...
   for (i=0; i<pldIn->uCount; i++)
      ...
      if (!(pfd = ReadFileInfo (pldIn, TRUE)))
         ...
         FreePLDESC (pldIn);
         FreePLDESC (pldOut);
         return SetErr (EBLERR_CANTREAD, "DescLib");
         }
      pfd->uMode = LIB;
      ulFilePos  = FilTell (pldOut->fp);
...
static UINT GetDescLib (PSZ pszLib, PSZ pszDesc) // fix1
   {
   PLDESC   pldIn;
...
static void MakeLibName (PSZ pszDest, PSZ pszSrc)
   ...
   // mdh: set global tmp file name to be on the same drive as the EBL
   if (pszSrc [1] == ':')
   	{
   	szTMPLIB [0] = pszSrc [0];
   	strcpy (szTMPLIB+1, ":~EBLTMP.@$@");
   	}
   else
   	strcpy (szTMPLIB, "~EBLTMP.@$@");
   	
   printf ("LIBNAME: '%s'. TMPLIB = '%s'", pszDest, szTMPLIB);
   }
...
static void SetFlags (UINT uFlags)
   ...
   bAPPEND     = uFlags & EBL_APPENDFILES; // mdh
   bFILEDESC   = FALSE;
   bSTOREONLY  = FALSE;
   bLIBDESC    = FALSE;
...
static UINT uiEblGetError =0;
static UINT uiEblAdd      =0;
static UINT uiEblMove     =0;
static UINT uiEblDelete   =0;
static UINT uiEblExtract  =0;
static UINT uiEblList     =0;
static UINT uiEblTest     =0;
static UINT uiEblDescribe =0;
static UINT uiEblGetDesc  =0;
...
UINT CCONV EblGetError (PSZ pszErrorStr)
   {
   uiEblGetError++;
   if (IsBadWritePtr (pszErrorStr, strlen (ppszEBL_ERRORS [GLOBALERROR])+1))
      return EBLERR_BUFFER_TOO_SMALL;
...
UINT CCONV EblAdd (PSZ pszLIB, PSZ pszFileSpec, UINT uFlags)
   ...
   SetLibErr (0);
   GLOBALERROR = 0;
   printf ("EblAdd Call %4.4d: pszLIB='%s', pszFileSpec='%s', uFlags = %x", uiEblAdd, pszLIB, pszFileSpec, uFlags);
   ...
   fList = FreePFDESC (fList);
   printf ("EblAdd: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
   return GLOBALERROR;
   }
...
UINT CCONV EblMove (PSZ pszLIB, PSZ pszFileSpec, UINT uFlags)
   ...
   SetLibErr (0);
   GLOBALERROR = 0;
   printf ("EblMove Call %4.4d: pszLIB='%s', pszFileSpec='%s', uFlags = %x", uiEblMove, pszLIB, pszFileSpec, uFlags);
   MakeLibName (szLIB, pszLIB);
   ...
   fList = FreePFDESC (fList);
   printf ("EblMove: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
   return GLOBALERROR;
   }
...
UINT CCONV EblDelete (PSZ pszLIB, PSZ pszFileSpec, UINT uFlags)
   ...
   SetLibErr (0);
   GLOBALERROR = 0;
   printf ("EblDelete Call %4.4d: pszLIB='%s', pszFileSpec='%s', uFlags = %x", uiEblDelete, pszLIB, pszFileSpec, uFlags);
   MakeLibName (szLIB, pszLIB);
   ...
   fList = FreePFDESC (fList);
   printf ("EblDelete: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
   return GLOBALERROR;
   }
...
UINT CCONV EblExtract (PSZ pszLIB, PSZ pszFileSpec, UINT uFlags)
   ...
   SetLibErr (0);
   GLOBALERROR = 0;
   printf ("EblExtract Call %4.4d: pszLIB='%s', pszFileSpec='%s', uFlags = %x", uiEblExtract, pszLIB, pszFileSpec, uFlags);
   MakeLibName (szLIB, pszLIB);
   ...
   fList = FreePFDESC (fList);
   printf ("EblExtract: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
   return GLOBALERROR;
   }
...
UINT CCONV EblList (PSZ pszLIB, PSZ pszFileSpec, PSZ pszOutFile)
   ...
   GLOBALERROR = 0;
   SetLibErr (0);
   printf ("EblList Call %4.4d: pszLIB='%s', pszFileSpec='%s', pszOutFile=%d", uiEblList, pszLIB, pszFileSpec, pszOutFile);
   MakeLibName (szLIB, pszLIB);
   ...
   fList = FreePFDESC (fList);
   printf ("EblList: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
	return GLOBALERROR;
   }

//UINT CCONV EblTest (PSZ pszLIB, BOOL bQuick)
UINT CCONV EblTest (PSZ pszLIB)
   ...
   printf ("EblTest Call %4.4d: pszLIB='%s'", uiEblTest, pszLIB);
   MakeLibName (szLIB, pszLIB);
   ...
   QuickTestLib (szLIB);
   fList = FreePFDESC (fList);
   printf ("EblTest: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
   return GLOBALERROR;
   }
...
UINT CCONV EblDescribe (PSZ pszLIB, PSZ pszDesc)
   ...
   printf ("EblDescribe Call %4.4d: pszLIB='%s', pszDesc='%s'", uiEblDescribe, pszLIB, pszDesc);
   MakeLibName (szLIB, pszLIB);
   ...
   fList = FreePFDESC (fList);
   printf ("EblDescribe: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
   return GLOBALERROR;
   }
...
UINT CCONV EblGetDesc (PSZ pszLIB, PSZ pszDesc, UINT usDesc)
   ...
   uiEblGetDesc++;
   printf ("EblGetDesc Call %4.4d; pszLIB='%s'", uiEblGetDesc, pszLIB);
   if (!pszDesc)
   	{
  		printf ("EblGetDesc: BUFFER TOO SMALL!.");
   	return SetErr (EBLERR_BUFFER_TOO_SMALL, "EblGetDesc1");
   	}
   ...
   MakeLibName (szLIB, pszLIB);
   if (!GetDescLib (szLIB, szTemp))
      {
	   i = strlen (szTemp) + 1;

   	if (usDesc < i || IsBadWritePtr (pszDesc, i))
   		{
   		printf ("EblGetDesc: BUFFER TOO SMALL!.");
   		return SetErr (EBLERR_BUFFER_TOO_SMALL, "EblGetDesc2");
   		}

		strcpy (pszDesc, szTemp);
		}
	else
		*pszDesc = 0;
	   
   printf ("EblGetDesc: Done. GLOBALERROR = %d, uLIBERR = %d, errno = %d.", GLOBALERROR, GetLibErr (), errno);
   return GLOBALERROR;
   }

static HANDLE hmodDll = 0;
static WORD wDataSeg = 0;
static WORD wHeap = 0;
static PSZ pszCmdLine = NULL;
static int iLibMainCount = 0;
static int iWEPCount = 0;
...
INT CCONV LibMain (HANDLE hmod, WORD dataseg, WORD heap, LPSTR cmdline)
   ...
   if (!hmodDll)
   	...
   	i = _creat (LOGFILE, _S_IREAD | _S_IWRITE);
   	if (i >= 0)
   		_close (i);
...
INT CCONV WEP (int res)
   ...
   res = res;  // get rid of anoying warning
   return 1;
   }
